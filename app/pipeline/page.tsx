      ...
      <div className="max-w-[1600px] mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <h1 className="text-5xl font-black tracking-tighter uppercase flex items-center gap-4 glitch-text">
              <LayoutDashboard className="w-10 h-10 text-primary" />
              Pipeline <span className="text-zinc-800 text-3xl">Prisma Studio</span>
            </h1>
          </div>
          
          <div className="bg-zinc-900/50 border border-white/5 p-4 rounded-2xl flex items-center gap-6 neon-glow">
            <div className="text-center">
              <span className="block text-[10px] uppercase font-bold text-zinc-500 tracking-widest">Activos</span>
              <span className="text-2xl font-black text-primary">{episodes?.filter(e => e.estado !== 'PUBLICADO').length || 0}</span>
            </div>
            <div className="w-px h-8 bg-zinc-800" />
            <div className="text-center">
              <span className="block text-[10px] uppercase font-bold text-zinc-500 tracking-widest">Publicados</span>
              <span className="text-2xl font-black text-accent">{episodes?.filter(e => e.estado === 'PUBLICADO').length || 0}</span>
            </div>
          </div>
        </div>
